...
function register () {
  ...
  this.on('navigationBuilt', ({ contentCatalog, navigationCatalog, uiCatalog }) => {
    ...
    contentCatalog.getComponents().forEach((c) => {
      ;(c.versions || []).forEach((v) => {
        ...
        try {
          ...
          const key = partialKey(c.name, v.version)
          putPartial(uiCatalog, key, html)
          known[key] = true
          count++
        } catch (e) {
          failed++
    ...
    putHelper(uiCatalog, 'navPartial', navPartialHelperSource(known))
...
function putHelper (uiCatalog, stem, source) {
  const path = 'helpers/' + stem + '.js'
  const existing = uiCatalog.findByType('helper').find((f) => f.path === path)
  if (existing) existing.contents = Buffer.from(source)
  else uiCatalog.addFile({ contents: Buffer.from(source), path, stem, type: 'helper' })
}

// Overwrites the UI bundle's helpers/navPartial.js (see there) with a version
// that knows exactly which nav-cv-* partials this build actually produced —
// so a page whose component-version isn't in `known` (its assembly failed
// above, or it's simply a component-version this build never covers) falls
// back to the UI bundle's generic 'nav-native' partial instead of a fatal
// "partial not found" build error.
function navPartialHelperSource (known) {
  return "'use strict'\n\n" +
    '// GENERATED by lib/nav-assembler.js — do not edit directly, edit there.\n' +
    'var KNOWN = ' + JSON.stringify(known) + '\n' +
    "function sanitize (s) { return String(s == null ? '' : s).replace(/[^a-z0-9]+/gi, '-').toLowerCase() }\n\n" +
    'module.exports = function (component, version) {\n' +
    "  if (!component || !version) return 'nav-generated'\n" +
    "  var key = 'nav-cv-' + sanitize(String(component) + '-' + String(version))\n" +
    "  return KNOWN[key] ? key : 'nav-native'\n" +
    '}\n'
}
...
function sectionHtml (title, items, counter) {
  // A <div role="heading" aria-level="3"> rather than a literal <h3>: this is
  // drawer chrome, not real page content, so it shouldn't compete with the
  // page's own headings in the document's native heading outline (screen
  // readers navigating by heading, or a linter scanning h1-h6) — but the ARIA
  // role/level keeps it discoverable as a heading for assistive tech.
  return '<section class="nav-section">' +
    '<div class="nav-section-title" role="heading" aria-level="3">' + esc(title) + '</div>' +
