...
// model instead of hand-authored markup.
//
...
function register () {
  ...
  this.on('navigationBuilt', ({ contentCatalog, navigationCatalog, uiCatalog }) => {
    let count = 0
    let failed = 0
    // Dedupes the "manifest include matched nothing" log below across the
    // whole build — buildDrawerHtml runs once per component-version (100+
    // times), and every run re-evaluates the same nav-manifest sections, so
    // without this the same message would repeat once per drawer.
    const warnedEmptyIncludes = new Set()

    contentCatalog.getComponents().forEach((c) => {
      ;(c.versions || []).forEach((v) => {
        // Scoped per component-version: one bad component/version shouldn't
        // silently abort assembly for every other one (previously a single
        // try/catch wrapped this entire loop).
        try {
          const html = buildDrawerHtml(contentCatalog, navigationCatalog, { component: c.name, version: v.version }, logger, warnedEmptyIncludes)
          putPartial(uiCatalog, partialKey(c.name, v.version), html)
          count++
        } catch (e) {
          failed++
          logger.error('nav-assembler failed for ' + c.name + '@' + v.version + ': ' + (e && e.stack ? e.stack : e))
        }
      })
    })
    ...
    try {
      ...
      putPartial(uiCatalog, 'nav-generated', buildDrawerHtml(contentCatalog, navigationCatalog, null, logger, warnedEmptyIncludes))
      putPartial(uiCatalog, 'nav-extension-products', buildExtensionProductsHtml(contentCatalog))
    } catch (e) {
      logger.error('nav-assembler failed building the default drawer/extensions list: ' + (e && e.stack ? e.stack : e))
    }

    logger.info('assembled ' + count + ' version-aware drawers' + (failed ? ' (' + failed + ' failed, see errors above)' : '') + ' + extensions product list')
  })
}
...
// item.url is only set by Antora for an internal xref it resolved against the
// content catalog. A plain external link (e.g. `https://github.com/...[Release
// 4.9]`, used throughout the legacy "older releases" nav.adoc files) has no
// item.url at all — the rendered <a href="..."> only exists inside
// item.content, which text() then strips as markup, silently discarding the
// destination entirely. Fall back to pulling the href out of item.content.
function externalHref (s) {
  var m = /<a\b[^>]*\bhref="([^"]*)"/i.exec(String(s == null ? '' : s))
  return m ? m[1] : null
}

function normalize (item) {
  var extHref = !item.url && externalHref(item.content)
  return {
    title: text(item.content),
    url: item.url || extHref || null,
    external: !!extHref,
    advanced: hasAdvanced(item.content),
    children: (item.items || []).map(normalize)
  }
}
...
// Flattened top-level items — an untitled menu is unwrapped (its items become
// top-level entries); a TITLED menu (e.g. the legacy "older releases" nav.adoc
// files' ".Axon Framework 4.x" / "3.x" / "2.x" / "1.x" blocks) becomes a group
// instead of silently losing its title, mirroring menuGroups() below. Used for
// category matching, which keys off individual entries' urls.
function topLevelItems (navigationCatalog, name, version) {
  ...
  ;(navigationCatalog.getNavigation(name, version) || []).forEach((menu) => {
    ...
    if (!node.title && node.children.length) {
      node.children.forEach((child) => { if (child.title || child.url || child.children.length) out.push(child) })
    } else if (node.title || node.url || node.children.length) {
      out.push(node)
    }
  })
  return out
...
function collectItems (contentCatalog, navigationCatalog, inc, focus, report, logger, warnedEmptyIncludes) {
  var items = []
  var resolved = resolveComponents(contentCatalog, navigationCatalog, inc, focus)
  if (!resolved.length && logger && warnedEmptyIncludes) {
    // Catches manifest typos (a component/componentGlob that matches nothing
    // in the whole catalog) — previously silent. Also fires for a component
    // that's legitimately absent from a partial/dev build, which is still
    // useful signal, not just noise.
    var key = JSON.stringify(inc)
    if (!warnedEmptyIncludes.has(key)) {
      warnedEmptyIncludes.add(key)
      logger.info('nav-manifest include matched no components: ' + key)
    }
  }
  resolved.forEach(({ name, version, title, url }) => {
    ...
    if (inc.groupByComponent) {
      ...
      items.push({ title: title, url: url, children: perComponent })
...
function renderItem (node, counter) {
  ...
  var extAttrs = node.external ? ' target="_blank" rel="noopener noreferrer"' : ''
  if (node.children && node.children.length) {
    ...
    var labelHtml = node.url
      ? '<a class="nav-parent-link" href="' + attr(node.url) + '"' + extAttrs + '><span>' + label + '</span></a>'
      : '<span>' + label + '</span>'
  ...
  if (node.url) {
    return '<li><a class="nav-link" data-href="' + attr(node.url) + '" href="' + attr(node.url) + '"' + extAttrs + '>' + label + '</a></li>'
  }
...
function buildDrawerHtml (contentCatalog, navigationCatalog, focus, logger, warnedEmptyIncludes) {
  var counter = { n: 0 }
  var report = { focusContributed: false }
  var sections = ''
  manifest.sections.forEach((section) => {
    var items = []
    ;(section.include || []).forEach((inc) => { items = items.concat(collectItems(contentCatalog, navigationCatalog, inc, focus, report, logger, warnedEmptyIncludes)) })
