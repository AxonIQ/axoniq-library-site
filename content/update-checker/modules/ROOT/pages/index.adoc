...
The AxonIQ update checker will retrieve available updates and known vulnerabilities for the Axon modules in use. This will then be communicated to the Axon Framework application’s logs. You can see such a sample down below.
...
**2. Collect data:**
To detect updates and vulnerabilities, the update checker collects data about your Axon Framework installation. This data is sent to AxonIQ and includes technical information about your environment, such as:
...
- Machine identifier and/or user name: A UUID stored in the user’s home directory, which is used to identify the installation.
...
[IMPORTANT]
====
We will never collect any application-specific, or business logic data.
====
...
This data provides insight into real-world usage of Axon Framework, helping us make informed decisions about development priorities, resource planning, and sustainability efforts. It also assists in ensuring compliance with licensing requirements for future commercial modules and End of Life exposure.

This in turn will allow AxonIQ to maintain the free and open-source core of Axon Framework while developing new enterprise features under a commercial license.
...
`disabled=true`
- **Spring Property** (Available since Axon Framework 4.12.2)
+
Add `axon.update-check.disabled=true` to your `application.properties` file (or the `application.yml` equivalent).
...
- Develop new enterprise features under a commercial, source-available, license.
...
Please review our link:https://www.axoniq.io/legal/privacy-policy[Privacy Policy] for more details.
