...
While we recommend installing Axon Server on virtual machines due to the better stability they provide, this guide will help you in installing Axon Server Cluster on Kubernetes.
...
. *Stateful set*: The stateful set for each node runs Axon Server with a stable hostname, necessary for the headless services.
...
. *Network Service*: For applications to be able to reach Axon Server and communication between nodes, Kubernetes requires a network service that resolves to the pod created by the stateful set.
. *Config Map*: A config map with the properties for Axon Server's configuration.
. *Secrets*: A kubernetes secret that safely store your license file.
...
== Step 2: Obtaining Axon Server License File
In order to run a cluster of Enterprise nodes, you need to provide your configration with an `axoniq.license` Axon Server license file.
You can obtain it contacting our sales department.

It is still possible to use AxonIQ Console: if you already have a License, please contact your sales-rep.

=== License File 

If you have a license file, we suggest to save this in a secure place, such as kubernetes secrets or the vault of your choice. 
In this guide we will store as kubernetes secret, but if you selected to form the cluster manually, you can also upload it directly to axonserver using the UI after the first node has been started.

[source,bash]
----
kubectl create secret generic axoniq-license -n axonserver --from-file=axoniq.license
----

=== AxonIQ Console

By connecting your Axon Server Enterprise to AxonIQ Console you can unlock additional functionality.
AxonIQ Console can also provide a license for your cluster.
...
. *Auto-clustering*: By defining which node is in charge of forming the cluster and initial contexts, it will automatically form.
. *Manual*: Initializing in the Axon Server dashboard by going into the Axon Server dashboard UI, you can initialize the cluster manually. You can optionally upload your file license here.
. *AxonIQ Console*: when connecting to AxonIQ Console, your server will receive all information automatically.

Based on which method you choose, different `axonserver-1.properties` `axonserver-2.properties` and `axonserver-3.properties` have to be put into the properties file:

[tabs]
======
Auto-clustering::
+
In the properties below we define that our node `axonserver-1.axonserver.svc.cluster.local` will be the one in charge of forming the cluster.
At startup the `default` context will be created: you can change this behaviour adding your context in `axoniq.axonserver.autocluster.contexts`
+
[tabs]
====
...
[source,properties]
----
include::example$kubernetes/axonserver-1.properties[tags=**;autoclustering;license;!console;!standalone]
----
--
...
[source,properties]
----
include::example$kubernetes/axonserver-2.properties[tags=**;autoclustering;license;!console;!standalone]
----
--
...
[source,properties]
----
include::example$kubernetes/axonserver-3.properties[tags=**;autoclustering;license;!console;!standalone]
----
--
====
Manual::
...
[source,properties]
----
include::example$kubernetes/axonserver-1.properties[tags=**;license;!autoclustering;!standalone;!console]
----
--
axonserver-2.properties:: 
+
...
[source,properties]
----
include::example$kubernetes/axonserver-2.properties[tags=**;license;!autoclustering;!standalone;!console]
----
--
axonserver-3.properties:: 
+
...
[source,properties]
----
include::example$kubernetes/axonserver-3.properties[tags=**;license;!autoclustering;!standalone;!console]
----
--
====
AxonIQ Console::
...
Auto-clustering::
+
If you select Auto-clustering you *must* provide license at startup, as a secret.
+
[tabs]
====
...
Manual::
+
If you select Manual, you can provide license as a secret or optionally upload to the node with the UI after the first node as started.
The example below is intended to use with a license file stored in a secret.
If you prefer to manually handle your license, you need to change your sts files by removing the parts referring to license under volumeMounts and volumes.
+
[tabs]
====
axonserver-sts-1.yaml:: 
...
AxonIQ Console::
+
If you use AxonIQ Console, license is provided by the service.
+
[tabs]
====
axonserver-sts-1.yaml:: 
