...
Some of the content sources are private repositories that needs authentication. If you are not an AxonIQ employee or trusted collaborator you likely don't have access to those. In such case you can still build a bigger part of the AxonIQ Library locally by making a copy of `playbook.yaml` and removing all the private content sources.

If you are a member of the https://github.com/AxonIQ[AxonIQ Organization on GitHub], you need to grant the build tool access to private repositories. To do so, you need to create a Personal Access Token (PAT).
...
With that in place you should be able to fetch content from private repositories and build the AxonIQ Library locally from the production playbook.
